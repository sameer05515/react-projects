...
public class MyResumeController {
    ...
    @PostMapping("/upsert")
    public ResponseEntity<List<MyResumeDto>> upsertMyResume(
            @RequestBody @NonNull List<MyResumeDto> myResumes
    ) {
    ...
    @GetMapping("/get")
    public ResponseEntity<MyResumeDto> getResumeByUniqueId(
            @RequestHeader("uniqueId") @NonNull String uniqueId
    ) {
//        try {

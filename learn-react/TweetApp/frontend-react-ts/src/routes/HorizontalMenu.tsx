...
export const HorizontalMenu = memo(function HorizontalMenu({ isAuthenticated, handleLogout }: Props) {
	...
	const handleLinkClick = useCallback((linkHeader: string) => {
					...
					.map(({ linkPath, linkHeader }, idx) => (
						<li
							...
							className={
								...
								(isPathActive(linkPath()) ? ' bg-menu-darker' : ' hover:bg-menu-darker') +
								(isPathActive(linkPath()) ? ' text-menu-yellow font-bold text-sm' : ' text-menu-light')
							}
						>
							<NavLink
								to={linkPath()}

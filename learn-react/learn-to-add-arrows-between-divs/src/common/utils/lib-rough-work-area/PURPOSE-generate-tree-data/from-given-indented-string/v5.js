...
const validateAndParseLines = (input) => {
  ...
  const adjustedLines = adjustLevels(lines, indentationValidation.initialIndentationLevel, indentationValidation.initialDifference);
...
const areIndentationsConsistent = (indentationLevels) => {
  ...
  return indentationLevels.slice(1).every(level => level >= initialIndentation);
};
...
const validateIndentation = (lines) => {
  ...
  const { index: firstNonZeroIndex, difference: initialDifference } = findFirstNonZeroDifference(indentationLevels);

  if (firstNonZeroIndex === -1) {
    if (lines.every(line => line.indentationLevel === initialIndentationLevel)) {
...
const adjustLevels = (lines, initialIndentationLevel, initialDifference) => {
  return lines.map(line => ({
    ...line,
    level: (line.indentationLevel - initialIndentationLevel) / initialDifference,
  }));
};

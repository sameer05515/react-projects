...
async function getQuestionsForParentId(parentId, selectFields) {
    try {
        const criteria = parentId
            ...
            : { parentId: { $in: [null, undefined, ""] }, isPrivate: { $in: [null, undefined, false] } };
...
const updateQuestionByUniqueId = async (uniqueId, questionData) => {
    try {
        ...
        // console.log(`categoryData : ${JSON.stringify(categoryData)}`)
        const { name, heading, smartContent, rating, linkedCategoryId, parentId, tags, children, order } =
            questionData;
...
const getQuestionsByTagId = async (tagId) => {
    try {
        ...
        console.error(error);
        throw new Error(`Error retrieving Questions with tagId ${tagId}: ${error.message}`);
    }
};
...
const updateAnswerByUniqueId = async (uniqueId, answerData) => {
    try {
        ...
        const { name, heading, smartContent, rating, linkedQuestionsId, tags, children, order } =
            answerData;

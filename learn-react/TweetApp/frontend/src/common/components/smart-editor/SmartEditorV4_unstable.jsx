...
const inputOutputMapping = {
  ...
  YAML_From_RT: {
    detailedName:
      "YAML format text, input from RawText. Will show parsing error if invalid YAML text given",
...
const SmartEditor = ({
  ...
  useEffect(() => {
    ...
    if (textOutputType === availableOutputTypes.TIS_to_SKELETON && content) {
    ...
    if (textareaRef.current) {
      ...
      textareaRef.current.style.height = `${
        textareaRef.current.scrollHeight * 1.3
      }px`;
    }
...
const SmartPreviewer = ({ data }) => {
  ...
  useEffect(() => {
    if (
      (textOutputType === availableOutputTypes.YAML ||
        textOutputType === availableOutputTypes.YAML_to_SKELETON) &&
      content
    ) {
      try {
        const yamlResponse=yaml.load(content);
        setYamlProcessedData(yamlResponse);
        if(textOutputType === availableOutputTypes.YAML_to_SKELETON){
  ...
  return (
    <>
      ...
      {textOutputType === availableOutputTypes.MARKDOWN && (
        <MarkdownComponent markdownText={content} />
      )}
      {textOutputType === availableOutputTypes.YAML && (
        <div>
      ...
      {textOutputType === availableOutputTypes.TIS_to_SKELETON &&
        resultData &&
        resultData.length > 0 && (
          <>
            <Tree

...
const updateTopicByUniqueId = async (uniqueId, topicData) => {
  try {
    ...
    const { parentId, name, description, smartContent, tags, occurenceDate, children } = topicData;
    topic.parentId = parentId || topic.parentId;
    topic.name = name || topic.name;
    topic.description = description || topic.description;
    topic.tags = tags && tags.length >= 0 ? tags : topic.tags;
    topic.occurenceDate = occurenceDate != null ? occurenceDate : topic.occurenceDate;
    topic.smartContent= smartContent;
    topic.updatedDate = new Date();
    topic = await topic.save();
    const childrenIds = children || [];
    // console.log(`childrenIds: ${childrenIds}`)
    await Topic.updateMany({ uniqueId: { $in: childrenIds } }, { parentId: topic.uniqueId });
    return topic;
  } catch (err) {
    console.error(err);
    throw err;
...
const getTopicByUniqueId = async (uniqueId) => {
  try {
    ...
    const sections = await TopicSection
      .find({ linkedTopicUniqueId: topic.uniqueId })
      .select({
        uniqueId: 1,
        name: 1,
...
async function getAllAncestors(parentId, ancestors = []) {
  ...
  //   return ancestors;
  // }
  ancestors.unshift({ name: topic.name, uniqueId: topic.uniqueId, parentId: topic.parentId }); // Add the name of the current topic to ancestors array
...
const searchTopics = async (searchString) => {
...
const getAllTopicSectionsById = async (linkedTopicUniqueId) => {
  const sections = await TopicSection.find({ linkedTopicUniqueId: linkedTopicUniqueId });
  return sections;
}

const getTopicSectionsById = async (linkedTopicUniqueId, sectionUniqueId) => {
  const section = await TopicSection.findOne({ linkedTopicUniqueId: linkedTopicUniqueId, uniqueId: sectionUniqueId });
  if (!section) {
    throw new Error(`TopicSection not found , linkedTopicUniqueId : ${linkedTopicUniqueId}, sectionUniqueId: ${sectionUniqueId}`);
  }
  return section;
}

const updateTopicSectionsById = async (topicUniqueId, sectionUniqueId, sectionData) => {
  console.log(`[Topic.service]: [updateTopicSectionsById]: sectionData : ${JSON.stringify(sectionData)}`);
  const { linkedTopicUniqueId, name, smartContent, order, tags } = sectionData;
  try {
    let section = await TopicSection.findOne({ linkedTopicUniqueId: linkedTopicUniqueId, uniqueId: sectionUniqueId });
    if (!section) {
      throw new Error(`TopicSection not found , linkedTopicUniqueId : ${linkedTopicUniqueId}, sectionUniqueId: ${sectionUniqueId}`);
    }
